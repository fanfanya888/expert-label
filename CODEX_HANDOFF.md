...
- 项目管理页已做一轮紧凑化：
  - 压缩项目列宽度和描述占高
  - 发布状态去掉与绿色标签重复的“已发布”文字
  - 状态与任务统计改成更紧凑的行内展示
- 项目管理页又做了一轮观感调整：
  - 页面内容宽度略收但不再过窄，保留更舒服的阅读宽度
  - 项目说明支持两行展示
  - 列表卡片和表格容器改成更圆角的视觉
  - 发布状态和任务统计恢复成紧凑竖排
- `single_turn_search_case` 的模板管理已改成：
  - 列表不再展示模板名称 / 模板说明，只展示前端顺序 `ID`
  - 列表顺序按当前模板集重新编号，删除后新增会补回当前可见序号，不跟数据库主键绑定
  - 新建模板弹窗不再要求填写模板名称 / 模板标识 / 模板说明
...
- `model_response_review` 的任务管理已与模板管理页对齐：
  ...
  - 去掉“模型回复已生成/未生成”这一列
  - 新建任务弹窗不再要求填写 `任务标识`，后端继续自动生成 `external_task_id`
- 首轮质检不再由管理员手动发起：
...
- `frontend/src/App.tsx` 已改成按路由懒加载，管理端和用户端页面不再一次性打进主包。
- `frontend/vite.config.ts` 已补基础拆包策略，当前固定拆出：
  - `react-vendor`
  - `router-vendor`
- 路由加载中的 fallback 已改成纯 CSS spinner，不再为了 loading 额外把更多 UI 组件压进入口。
...
- `single_turn_search_case` 模板默认名与管理端顺序 ID 这次也没有新增迁移。
...
- 路由懒加载后 chunk 数量明显增多，属于预期；如果后续再引入大体量图表或编辑器，需要继续按页面控制依赖边界。
